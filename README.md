...
**0.5.3 (26) — private-build candidate, September 6, 2026.** Schedule-first shortcuts replace the overlapping Transactions and Manage roster pages. Adds / Drops combines available players and owned-player drops; Injured Reserve has its own capacity and eligibility view. The shared [standings pattern](docs/standings-pattern.md) uses numeric places such as “6–2 · 1st in Warner,” with correctly scoped ranks and no fabricated preseason first place. Season points and compact contextual actions remain. This is not a public or TestFlight release. See [current status](docs/current-status.md) for exact verification, pending work and device evidence.

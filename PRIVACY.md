...
- Last-loaded scores/projections, lineup, standings, team identity, own-team season roster and Board list summaries may be retained in one size-bounded display file for up to seven days per section. It does not retain full Board posts or pending bids/trade offers. A separate daily metadata file retains MFL's league export, which may include owner names/contact fields and bid balances supplied by MFL. It is reused only after fresh membership verification; balance display and submissions require stricter freshness.

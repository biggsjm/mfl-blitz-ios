...
The block editor follows Lineup's staged promotion/demotion and pinned review pattern. Native tests must prove moving a row does not submit, Cancel review preserves edits, final submit shows a receipt only after both sheets finish dismissing, and unchanged readback cannot submit again. Settings is on My Team from build 34; Scores retains only the Week control. Create trade sits directly below the Trades mode selector, so geometry assertions should reference that selector without weakening visibility or target-size requirements.

Build 35 distinguishes a blank new block from deliberate last-player removal. Cover save/resume of an empty removal draft, cancelable removal review, both explicit empty form fields, absent/empty fresh readback, unchanged/failed readback, fresh permission/baseline rejection and timeout/relaunch without replay. The fixture's accepted empty replacement is not proof of MFL server behavior; real intended removal remains an owner check. Never substitute a roster/drop import or a guessed player-ID sentinel.

Reminder tests cover future-only absolute triggers, the 14-day/32-alert budget, overrides, stale reads, permission denial, failed preference restoration and disconnect. Native Preview must never request OS notification permission or create an actual notification/calendar event. The app embeds `MFLBlitzLiveActivity.appex`; shared attributes must compile in both targets, remain small and contain no credentials. Test fresh current-week playing-starter eligibility and explicit staleness; compilation/model tests are not evidence of real game-day Lock Screen delivery. Josh approved private MFL → Hephaestus → ActivityKit background updates on September 13. Follow the [service/privacy plan](docs/background-scoring.md): token lifecycle, owner-only access, bounded polling/retention and no MFL credential transfer. Keep ordinary non-push builds available until Apple provisioning is configured; never equate APNs acceptance with observed device delivery.

...
**0.5.1 (20) candidate** extends the approved [features 1–5](player-tools-plan.md): official availability, on-demand league-scored history, synced watchlist, reviewed FCFS add/drop and IR activation/deactivation. Build **0.5.0 (19)** installed and launched on Josh's iPhone September 6. Josh then reported enabled IR controls for ineligible players and 429 cooldowns on Lineup/My Team. Build 20 adds early IR eligibility gating, section-local refresh, cached browse reads, deferred history, per-host cooldowns and 1.25-second request spacing. It is undergoing final regression/CI/device verification.

Wire validation: Josh supplied abilities, empty/singleton watchlist responses and player 9431's owned status for franchise 0008. Public availability feeds were checked; last season's nonempty points-allowed response verified position totals. No real roster/trade/waiver/lineup/board writes were made by automated QA. The complete local regression passed 170 app/UI functions / 199 executions, zero failures/runtime warnings. The latest core suite passed 74 tests after adding acquisition-flag regressions. Final Dark UI/source follow-up and older-toolchain CI are recorded separately when complete.

Actual Light/Dark screenshots were reviewed for watchlist, research, explicit add/drop review, IR confirmation and largest-text identity/action. All four then-existing player-tools journeys and all 143 then-current app unit functions passed. A new trade research/back assertion exposed mixed navigation styles in the composer; typed composer routes fixed the picker return and the focused trade journey passed. The IR/rate-limit feedback adds new tests; earlier results are not presented as the final build-20 result.

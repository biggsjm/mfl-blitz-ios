...
Build 40 adds the approved [scoring system](scoring-visual-system.md) and contextual player destination. From a matchup, an active or finished player opens **Week N / Player**, defaulting to the inspected week's points and any MFL-provided scoring text. Before kickoff, for an explicit bye, or when status is unknown, it remains the normal card without the segments. The week summary renders from the tapped response immediately; the profile's optional requests start on selecting Player. Normal global search, lineup and roster entry points remain card-first. The existing Back path and scope boundaries are unchanged. No structured NFL box score is invented when MFL omits stats.

Owner names appear directly below team names in smaller, secondary text, reusing league metadata with no new request. Slot/FLEX labels omit acronyms and positional subtotals: overall team totals and individual player points are the useful comparisons. September 9's [compact scoring refinement](compact-scoring.md) moves these into one paired-row table and a smaller team header. Bench remains collapsed and does not affect team totals; VoiceOver player labels retain team context.

Build 36 introduced compact NFL game context: `@ NYG · Sun 12:00 PM`, `vs NYG · Live`, `@ NYG · Final`, or an explicit `Bye week`. Build 40 uses a green dot plus Live text, with cached/delayed state qualified as Last known. Name/actual points remain primary and projections are separately labeled. Team/opponent and day/time use separate caption lines; the day and time stay together, with a stacked score/context fallback for narrow widths. Starting lineups explains the device time zone once (for example, `Times in CT`); locale-aware formatting honors 12/24-hour preferences and daylight saving. Accessibility sizes retain the stacked comparison layout and spoken player labels include game context.
...
This original increment introduced no Players tab or second lineup editor. September 9's approved [global search](player-search.md) subsequently adds a shared player-search sheet that routes to this same player card. Week 1 live validation and Week 2 distribution gates remain separate.
